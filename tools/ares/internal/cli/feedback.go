...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".ares-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("ARES_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("ARES_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "ares-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.ares-pp-cli/feedback.jsonl.
When ` + "`ARES_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`ARES_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  ares-pp-cli feedback list
  ares-pp-cli feedback list --limit 5
  ares-pp-cli feedback list --json`,
