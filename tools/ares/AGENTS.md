# ARES Printed CLI Agent Guide

This directory is a generated `ares-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
ares-pp-cli doctor --json
ares-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
ares-pp-cli which "<capability>" --json
ares-pp-cli <command> --help
```
...
```bash
ares-pp-cli <command> --help
ares-pp-cli <command> --dry-run --agent
```
