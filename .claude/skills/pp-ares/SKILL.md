...
This skill drives the `ares-pp-cli` binary. **You must verify the CLI is installed before invoking any command from this skill.**

**In this repo (Cherrypick) the binary is vendored** — prefer the local build over a global install. From the repo root:

```bash
bun run ares:build      # -> tools/ares/bin/ares-pp-cli
tools/ares/bin/ares-pp-cli --version
bun run ares:install    # optional: put it on $PATH
```

If you are outside this repo and the binary is missing, install it first:
...
## Domain knowledge & gotchas (verified)

Things the official API docs and `--help` don't tell you, learned the hard way. Read this before composing a query.

### What ARES does and doesn't have
- **No contacts in the basic lookup.** `ekonomicke-subjekty vrat-ekonomicky-subjekt` / `vyhledej` (RES/VR/RŽP) return **no website, email, or phone**.
- **Contacts DO exist in 3 source registers** — query the source-specific endpoint by IČO:
  - **NRPZS** (healthcare facilities) → `www`, `email`, `telefon`, `fax` — `ekonomicke-subjekty-nrpzs vrat-ekonomicky-subjekt-nrpzs <ico>`
  - **RS** (schools register) → `www`, `email[]`, `telefon` — `ekonomicke-subjekty-rs vrat-ekonomicky-subjekt-rs <ico>`
  - **ROS** → `telefon`, `email` (no www) — `ekonomicke-subjekty-ros vrat-ekonomicky-subjekt-ros <ico>`
  - For ordinary s.r.o./a.s. there is no website anywhere in ARES — use web search instead.
- **Company size** = `kategoriePoctuPracovniku` (employee-count bucket). Returned **only by the RES endpoint** `ekonomicke-subjekty-res vrat-ekonomicky-subjekt-res <ico>`, NOT by the basic lookup. Many small firms report `000 = Neuvedeno`.

### Search filter (`ekonomicke-subjekty vyhledej`)
- Filters available: `ico`, `obchodni-jmeno`, `pravni-forma`, `pravni-forma-ros`, `cz-nace`, `financni-urad`, and `sidlo-*` (address). **That's it.**
- **Cannot filter by company size** server-side — `kategoriePoctuPracovniku` is not a filter. To filter by size: `vyhledej` → collect IČOs → call `-res` per IČO → filter locally.
- **Location is by numeric code, not name** (`--sidlo-kod-obce`, `--sidlo-kod-ulice`, …). `--sidlo-textova-adresa` is fulltext and will mix up same-named towns (e.g. **Frýdlant u Liberce kód 564028** vs **Frýdlant nad Ostravicí kód 598143**). Prefer the obec code for clean results.
- **1 000-result cap.** Too broad a query returns HTTP 400 `VYSTUP_PRILIS_MNOHO_VYSLEDKU`. Narrow it (street, č.p., legal form) or page with `--start`/`--pocet`.
- **Codes, not text**, for `--pravni-forma` and `--cz-nace`. Look codes up via `ciselniky-nazevniky vyhledej --kod-ciselniku <Name> --agent`:
  - **PravniForma**: `101` OSVČ · `111` v.o.s. · `112` s.r.o. · `113` k.s. · `121` a.s. · `205` družstvo
  - **KategoriePoctuPracovniku**: `110` 0 · `120` 1–5 · `130` 6–9 · `210` 10–19 · `230` 25–49 · `240` 50–99 · `330` 250–499 · `510` 10000+
- Multi-value filters accept a comma list or a bare value: `--pravni-forma 112,121`, `--cz-nace 6210`. (These map to JSON string arrays internally.)

### Common workflow: "find companies in location X, form Y, size Z"
1. `ekonomicke-subjekty vyhledej --sidlo-kod-obce <code> --pravni-forma 112,121 --pocet 100 --agent` → list of IČOs
2. For each IČO: `ekonomicke-subjekty-res vrat-ekonomicky-subjekt-res <ico> --agent` → read `kategoriePoctuPracovniku`, filter locally
3. Optionally `enrich <ico…>` for address+DIČ, and the NRPZS/RS/ROS endpoint for contacts.
